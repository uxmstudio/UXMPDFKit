...
internal class UXMArray: UXMObject {
    ...
    func dictionaryAtIndex(_ index: Int) -> UXMDictionary? {
        var dictionary: CGPDFDictionaryRef? = nil
        if CGPDFArrayGetDictionary(arr, index, &dictionary) {
            return UXMDictionary(dictionaryRef: dictionary!)
        }
        return nil
    }
    
    func arrayAtIndex(_ index: Int) -> UXMArray? {
        var array: CGPDFArrayRef? = nil
        if CGPDFArrayGetArray(arr, index, &array) {
            return UXMArray(arrayRef: array!)
        }
        return nil
    }
...
extension UXMArray: NSCopying {
    func copy(with zone: NSZone?) -> Any {
        return Swift.type(of: self).init(arrayRef: arr)
    }
}

extension UXMArray: Sequence {
